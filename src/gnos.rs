...
fn main(args: [str])
{
	...
	let subjects_v: server::response_handler = |_settings, request, response| {get_subjects::get_subjects(state_chan, request, response)};	// need a unique pointer (bind won't work)
