...
priv fn update_got(state_chan: comm::Chan<model::Msg>, winterfell_loyalty: ~str, wl: f64, kings_landing_loyalty: ~str, kl: f64, poll_rate: u16)
{
	...
	comm::send(state_chan, model::UpdateMsg(~"primary",
		|store, _data|
		{
			...
			store.replace_triple(~[], {subject: copy winterfell_loyalty2, predicate: ~"gnos:level", object: FloatValue(wl)});
			store.replace_triple(~[], {subject: copy kings_landing_loyalty2, predicate: ~"gnos:level", object: FloatValue(kl)});
			true
		}, ~""));
